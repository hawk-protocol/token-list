...
const LOGO_URI_BASE =
  "https://raw.githubusercontent.com/hawkswap/default-token-list/master";
...
const main = async () => {
  ...
  const previousTokenList = requireOrNull(
    __dirname,
    "../hawkswap.token-list.json"
  );
  const previousExperimentalTokenList = requireOrNull(
    __dirname,
    "../hawkswap-experimental.token-list.json"
  );
  ...
  await fs.writeFile(
    __dirname + "/../hawkswap.token-list.json",
    JSON.stringify(tokenList, null, 2)
  );

  await fs.writeFile(
    __dirname + "/../hawkswap-experimental.token-list.json",
